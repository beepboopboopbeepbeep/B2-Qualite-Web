      ...
      <div class="rounded-2xl border border-zinc-800 bg-zinc-900/30 overflow-hidden">
        ...
        <div class="p-6">
          <div v-if="activeTab === 'preview'" class="space-y-4">
            <div class="text-sm text-zinc-400">L'utilisateur peut vérifier sa saisie.</div>
            <form class="max-w-md rounded-xl border border-zinc-800 bg-zinc-950 p-5">
              <div class="space-y-1.5">
                <label for="pwd-show" class="block text-sm font-medium text-zinc-200">Mot de passe</label>
                <div class="relative">
                  <input
                      :type="showPassword ? 'text' : 'password'"
                      id="pwd-show"
                      value="MonSuperMotDePasse!"
                      class="w-full rounded-md border border-zinc-700 bg-zinc-900 pl-3 pr-10 py-2 text-sm text-zinc-100 focus:border-indigo-500 focus:outline-none"
                  />
                  <button
                      type="button"
                  ...
                  >
                    <span class="text-xs uppercase font-medium">{{ showPassword ? 'Masquer' : 'Afficher' }}</span>
                  </button>
                </div>
              </div>
            </form>
          ...
          <div v-else>
  <pre class="rounded-xl bg-zinc-950 p-5 overflow-x-auto text-sm text-zinc-100"><code>&lt;!-- Règle 76 : Affichage du mot de passe --&gt;
&lt;div class="relative"&gt;
  &lt;input
    :type="showPassword ? 'text' : 'password'"
    id="pwd"
