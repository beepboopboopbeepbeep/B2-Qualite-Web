      ...
      <div class="rounded-2xl border border-zinc-800 bg-zinc-900/30 overflow-hidden">
        ...
        <div class="p-6">
          <div v-if="activeTab === 'preview'" class="space-y-4">
            <div class="text-sm text-zinc-400">Respect de la proximité (Loi de Gestalt) : les labels sont collés aux champs.</div>
            <form class="max-w-md rounded-xl border border-zinc-800 bg-zinc-950 p-5 space-y-6">
              <div>
                <label for="ville" class="block text-sm font-medium text-zinc-200 mb-1">Ville de destination</label>
                <input type="text" id="ville" class="w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100" />
              </div>

              <div>
                <span class="block text-sm font-medium text-zinc-200 mb-2">Classe de voyage</span>
                <div class="flex gap-4">
                  <div class="flex items-center gap-2">
                    <input type="radio" id="eco" name="classe" class="accent-indigo-500" />
                    <label for="eco" class="text-sm text-zinc-300">Économique</label>
                  </div>
                  <div class="flex items-center gap-2">
                    <input type="radio" id="business" name="classe" class="accent-indigo-500" />
                    <label for="business" class="text-sm text-zinc-300">Affaires</label>
                  </div>
                </div>
              </div>
            </form>
          ...
          <div v-else>
  <pre class="rounded-xl bg-zinc-950 p-5 overflow-x-auto text-sm text-zinc-100"><code>&lt;!-- Règle 77 : Proximité visuelle --&gt;

&lt;!-- BON : Marge basse (mb-1) petite entre label et input --&gt;
&lt;label for="ville" class="mb-1 block"&gt;Ville&lt;/label&gt;
&lt;input id="ville" ... /&gt;

&lt;!-- BON : Flexbox gap-2 pour coller label radio à son bouton --&gt;
&lt;div class="flex items-center gap-2"&gt;
  &lt;input type="radio" id="eco" /&gt;
