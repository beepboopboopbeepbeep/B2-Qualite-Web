      ...
      <div class="rounded-2xl border border-zinc-800 bg-zinc-900/30 overflow-hidden">
        ...
        <div class="p-6">
          ...
          <div v-if="activeTab === 'preview'" class="space-y-4">
            <div class="text-sm text-zinc-400">La limite est indiquée clairement dans le label.</div>
            <form class="max-w-md rounded-xl border border-zinc-800 bg-zinc-950 p-5">
              <div class="space-y-1.5">
                <div class="flex justify-between items-baseline">
                  <label for="bio" class="block text-sm font-medium text-zinc-200">
                    Biographie courte
                  </label>
                  <span class="text-xs text-zinc-500">140 caractères max.</span>
                </div>
                <textarea id="bio" rows="3" maxlength="140" class="w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 focus:border-indigo-500 focus:outline-none"></textarea>
                <div class="text-right text-xs text-zinc-600">0 / 140</div>
              </div>
            </form>
          </div>
          <div v-else>
  <pre class="rounded-xl bg-zinc-950 p-5 overflow-x-auto text-sm text-zinc-100"><code>&lt;!-- Règle 74 : Limite de caractères --&gt;
&lt;div class="flex justify-between"&gt;
  &lt;label for="bio"&gt;Biographie courte&lt;/label&gt;
  &lt;span class="text-xs text-zinc-500"&gt;140 caractères max.&lt;/span&gt;
&lt;/div&gt;

&lt;textarea id="bio" maxlength="140"&gt;&lt;/textarea&gt;</code></pre>
          </div>
          <!-- FIN CONTENU EXEMPLE -->
        </div>
      </div>
