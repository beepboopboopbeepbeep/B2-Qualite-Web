      ...
      <div class="rounded-2xl border border-zinc-800 bg-zinc-900/30 overflow-hidden">
        ...
        <div class="p-6">
          <div v-if="activeTab === 'preview'" class="space-y-4">
            <div class="text-sm text-zinc-400">Le texte d'aide est immédiatement rattaché au champ, pas rejeté en bas de page.</div>
            <form class="max-w-md rounded-xl border border-zinc-800 bg-zinc-950 p-5">
              <div class="space-y-1">
                <label for="cvv" class="block text-sm font-medium text-zinc-200">Cryptogramme visuel (CVV)</label>
                <p id="cvv-help" class="text-xs text-zinc-500 mb-1">Les 3 derniers chiffres au dos de votre carte.</p>
                <input type="text" id="cvv" aria-describedby="cvv-help" class="w-24 rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100" placeholder="123" />
              </div>
            </form>
          </div>
          <div v-else>
  <pre class="rounded-xl bg-zinc-950 p-5 overflow-x-auto text-sm text-zinc-100"><code>&lt;!-- Règle 78 : Infos contextuelles rattachées --&gt;
&lt;div class="space-y-1"&gt;
  &lt;label for="cvv"&gt;CVV&lt;/label&gt;

  &lt;!-- Aide placée juste entre le label et l'input --&gt;
  &lt;p id="cvv-help" class="text-xs text-zinc-500"&gt;
    Les 3 derniers chiffres au dos.
  &lt;/p&gt;

  &lt;input id="cvv" aria-describedby="cvv-help" /&gt;
&lt;/div&gt;</code></pre>
          </div>
        </div>
      </div>
