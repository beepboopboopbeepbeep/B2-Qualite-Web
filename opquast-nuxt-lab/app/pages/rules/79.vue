      ...
      <div class="rounded-2xl border border-zinc-800 bg-zinc-900/30 overflow-hidden">
        ...
        <div class="p-6">
          <div v-if="activeTab === 'preview'" class="space-y-4">
            <div class="text-sm text-zinc-400">Double signalement : résumé global en haut + erreur sur le champ.</div>
            <form class="max-w-md rounded-xl border border-zinc-800 bg-zinc-950 p-5 space-y-4">

              <div role="alert" class="rounded-md bg-red-900/20 border border-red-800 p-3 flex gap-3">
                <span class="text-red-500 text-lg">⚠️</span>
                <div>
                  <h3 class="text-sm font-medium text-red-200">Le formulaire contient des erreurs</h3>
                  <ul class="text-xs text-red-300 mt-1 list-disc pl-4">
              ...
              <div class="space-y-1.5">
                <label for="mail-err" class="block text-sm font-medium text-zinc-200">
                  Email
                </label>
                <input
                    type="email"
                    ...
                    value="jean.dupont@"
                    aria-invalid="true"
                    aria-describedby="mail-error-msg"
                    class="w-full rounded-md border border-red-500 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:ring-1 focus:ring-red-500"
                />
                <p id="mail-error-msg" class="text-xs text-red-400 font-medium">Format d'email incorrect.</p>
              </div>
            </form>
          </div>
          <div v-else>
  <pre class="rounded-xl bg-zinc-950 p-5 overflow-x-auto text-sm text-zinc-100"><code>&lt;!-- Règle 79 : Signalement des erreurs --&gt;

&lt;!-- 1. Résumé global --&gt;
&lt;div role="alert" class="bg-red-900/20 border-red-800"&gt;
  &lt;h3&gt;Le formulaire contient des erreurs&lt;/h3&gt;
  &lt;ul&gt;&lt;li&gt;Email invalide&lt;/li&gt;&lt;/ul&gt;
&lt;/div&gt;

&lt;!-- 2. Erreur contextuelle sur le champ --&gt;
...
/&gt;
&lt;p class="text-red-400"&gt;Format incorrect&lt;/p&gt;</code></pre>
          </div>
        </div>
      </div>
    </section>
