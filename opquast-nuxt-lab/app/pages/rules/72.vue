      ...
      <div class="rounded-2xl border border-zinc-800 bg-zinc-900/30 overflow-hidden">
        ...
        <div class="p-6">
          ...
          <div v-if="activeTab === 'preview'" class="space-y-4">
            <div class="text-sm text-zinc-400">Les formats attendus sont explicites dans le label.</div>
            <form class="space-y-5 max-w-md rounded-xl border border-zinc-800 bg-zinc-950 p-5" onsubmit="event.preventDefault();">
              <div class="space-y-1.5">
                <label for="date" class="block text-sm font-medium text-zinc-200">
                  Date de naissance <span class="text-zinc-500">(JJ/MM/AAAA)</span>
                </label>
                <input type="text" id="date" placeholder="ex: 14/07/1990" class="w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-indigo-500 focus:outline-none" />
              </div>
              <div class="space-y-1.5">
                <label for="phone" class="block text-sm font-medium text-zinc-200">
                  Téléphone mobile <span class="text-zinc-500">(10 chiffres, sans espaces)</span>
                </label>
                <input type="tel" id="phone" placeholder="0612345678" class="w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-indigo-500 focus:outline-none" />
              </div>
            </form>
          </div>
          <div v-else>
            <pre class="rounded-xl bg-zinc-950 p-5 overflow-x-auto text-sm text-zinc-100"><code>&lt;!-- Règle 72 : Format indiqué dans le label --&gt;
&lt;div class="space-y-1.5"&gt;
  &lt;label for="date" class="block text-sm font-medium text-zinc-200"&gt;
    Date de naissance &lt;span class="text-zinc-500"&gt;(JJ/MM/AAAA)&lt;/span&gt;
  &lt;/label&gt;
  &lt;input type="text" id="date" placeholder="ex: 14/07/1990" ... /&gt;
&lt;/div&gt;
