      ...
      <div class="rounded-2xl border border-zinc-800 bg-zinc-900/30 overflow-hidden">
        ...
        <div class="p-6">
          ...
          <div v-if="activeTab === 'preview'" class="space-y-4">
            <div class="text-sm text-zinc-400">L'utilisateur est averti que la casse est importante.</div>
            <form class="max-w-md rounded-xl border border-zinc-800 bg-zinc-950 p-5">
              <div class="space-y-1.5">
                <label for="promo" class="block text-sm font-medium text-zinc-200">
                  Code Promotionnel <span class="text-amber-500 text-xs font-normal ml-1">(Saisir en majuscules)</span>
                </label>
                <input type="text" id="promo" class="w-full uppercase rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 focus:border-indigo-500 focus:outline-none" placeholder="PROMO2026" />
              </div>
            </form>
          </div>
          <div v-else>
  <pre class="rounded-xl bg-zinc-950 p-5 overflow-x-auto text-sm text-zinc-100"><code>&lt;!-- Règle 73 : Indication de casse --&gt;
&lt;label for="promo"&gt;
  Code Promotionnel
  &lt;span class="text-amber-500"&gt;(Saisir en majuscules)&lt;/span&gt;
&lt;/label&gt;

&lt;!-- L'attribut CSS ou classe uppercase aide visuellement mais ne remplace pas l'instruction --&gt;
&lt;input type="text" id="promo" class="uppercase" placeholder="PROMO2026" /&gt;</code></pre>
          </div>
          <!-- FIN CONTENU EXEMPLE -->
        </div>
      </div>
